...
void MouseRotator::poll(GLFWwindow* window) {
	...
	if (alt || rotStarted) {
		if (currentLeft && lastLeft) {  // If a left button drag is in progress
			...
			phi += PI * moveX / windowWidth;  // Longest drag rotates 180 degrees
			if (phi >= PI * 2.0 + 0.001f) phi = 0.0f;
			if (phi <= 0.0 - 0.001f) phi = PI * 2.0f;

			theta += PI * moveY / windowHeight;  // Longest drag rotates 180 deg
			if (theta >= PI * 2.0 + 0.001f) theta = 0.0f;
			if (theta <= 0.0f - 0.001f) theta = PI * 2.0f;
			rotStarted = true;
		}
		else
			rotStarted = false;
